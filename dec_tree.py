...
class DecisionTreeRegressor:
    ...
    def _find_split(self, bud):
        ...
        # Handle the case when self.C is None
        if self.C is None:
            eligible_predictors = np.arange(self.D)
        else:
            eligible_predictors = np.random.permutation(np.arange(self.D))[:self.C]
        ...
        for predictor in eligible_predictors:
            ...
            if dtype == 'quant':
                if self.C is None:
                    thresholds = np.linspace(np.min(X_sub), np.max(X_sub), num=self.D + 1)[1:-1]  # Use all features
                else:
